...
const AuthModal: React.FC<AuthModalProps> = ({ isOpen, onClose, initialMode = 'signin' }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      alert(mode === 'signup' ? t('auth.signUpError') : t('auth.signInError'))
    } finally {
      setIsLoading(false)
    }
  }
      ...
      <div className="bg-white rounded-lg p-8 w-full max-w-md mx-4 max-h-[90vh] overflow-y-auto">
        ...
        <div className="mb-6">
          <button
            ...
            disabled={isLoading}
            className="w-full flex items-center justify-center space-x-3 bg-white border border-gray-300 rounded-md py-3 px-4 hover:bg-gray-50 transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
          >
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div>
            ...
            <div className="relative">
              ...
              <input
                ...
                className="w-full pl-10 pr-12 py-3 border border-gray-300 rounded-md focus:ring-2 focus:ring-orange-500 focus:border-orange-500"
          ...
          >
            {isLoading ? t('auth.pleaseWait') : (mode === 'signin' ? t('navigation.signIn') : t('auth.createAccount'))}
          </button>
        </form>
        ...
        <div className="mt-6 text-center">
          <p className="text-sm text-gray-600">
            ...
            >
              {mode === 'signin' ? t('navigation.signUp') : t('navigation.signIn')}
            </button>
          </p>
        </div>
        ...
        {mode === 'signup' && (
          <div className="mt-4 text-center">
            <p className="text-xs text-gray-500">
              {t('auth.agreeToTerms')}{' '}
              <a href="/terms" className="text-orange-600 hover:text-orange-700">{t('auth.termsOfService')}</a>
              {' '}{t('common.and')}{' '}
              <a href="/privacy" className="text-orange-600 hover:text-orange-700">{t('auth.privacyPolicy')}</a>
            </p>
          </div>
        )}
      </div>
